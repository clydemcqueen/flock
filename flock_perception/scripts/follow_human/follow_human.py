...
DISTANCE_KEEP_AWAY = 250  # unit ? (from Detection2DArray) # higher is closer
DISTANCE_KEEPING_RANGE = 20 # unit ? (from Detection2DArray) 
KP = 0.005 #0.00375 # u/error i.e. 0.5/200
SLOW_TURN = 0.2
FAST_TURN = 0.5
FAST_UP = 0.3
...
class FollowHuman:
	...
	self.is_commandable = True#False # only publish commands when airborne
	#
	...
	if human is None:
	    ...
	    cx, cy, bound_height, bound_width = self.get_object_attrs(human)

	    # show center
	    cv2.rectangle(img,(int(cx-5),int(cy+5)),(int(cx+5),int(cy-5)),(255,255,255),1)
	    # show bound size
	    cv2.putText(img, 'bound_height: '+str(bound_height), (40,620), 
	...
	#print cx, cy
	bound_width = obj.bbox.size_x
	bound_height = obj.bbox.size_y		    
	return cx, cy, bound_height, bound_width
	...
	if cx < (self.img_width/2 - WIDTH_KEEPING_FOV):
		...
		lx = -ucontrol
  	    ...
  	    #if cy < (self.img_height*2/3 - HEIGHT_KEEPING_FOV):
	    #    lz = FAST_UP 
	    #    self.cmd += 'move up'
	    #elif cy > (self.img_height*2/3 + HEIGHT_KEEPING_FOV):	
	# 	lz = -FAST_UP
	#	self.cmd += 'move down'
